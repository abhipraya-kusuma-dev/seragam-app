...
@section('content')
    ...
    <section class="flex justify-between py-10 px-[46px]">
        <div class="flex flex-col gap-5">
            <div class="border rounded-xl border-black px-4 py-2" style="box-shadow: 2px 4px 6px rgb(177, 177, 177)">
                <form class="flex flex-col gap-4">
                    ...
                    <div class="flex items-center gap-4">
                        <label for="sd"
                            class="has-[:checked]:border-2 has-[:checked]:border-[#FF5656] has-[:checked]:bg-white has-[:checked]:text-[#FF5656] border-2 border-transparent select-none text-center rounded-[8px] py-1 px-8 bg-[#FF5656] text-white"
                        ...
                        <label for="smp"
                            class="has-[:checked]:border-2 has-[:checked]:border-[#3485FF] has-[:checked]:bg-white has-[:checked]:text-[#3485FF] border-2 border-transparent select-none text-center rounded-[8px] py-1 px-7 bg-[#3485FF] text-white"
                            style="box-shadow: 2px 4px 5px rgb(177, 177, 177)">
                            SMP
                        ...
                        <label for="sma"
                            class="has-[:checked]:border-2 has-[:checked]:border-[#2BCB4E] has-[:checked]:bg-white has-[:checked]:text-[#2BCB4E] border-2 border-transparent select-none text-center rounded-[8px] py-1 px-7 bg-[#2BCB4E] text-white"
                            style="box-shadow: 2px 4px 5px rgb(177, 177, 177)">
                            SMA
                        ...
                        <label for="smk"
                            class="has-[:checked]:border-2 has-[:checked]:border-[#DC6B19] has-[:checked]:bg-white has-[:checked]:text-[#DC6B19] border-2 border-transparent select-none text-center rounded-[8px] py-1 px-7 bg-[#DC6B19] text-white"
                            style="box-shadow: 2px 4px 5px rgb(177, 177, 177)">
                            SMK
                    ...
                    <div class="flex items-center gap-4">
                        <label for="cowo"
                            class="has-[:checked]:border-2 has-[:checked]:border-[#3485FF] has-[:checked]:bg-white has-[:checked]:text-[#3485FF] border-2 border-transparent select-none rounded-[8px] py-1 px-7 bg-[#3485FF] text-white"
                            style="box-shadow: 2px 4px 5px rgb(177, 177, 177)">
                            Pria
                        ...
                        <label for="cewe"
                            class="has-[:checked]:border-2 has-[:checked]:border-[#FF34C6] has-[:checked]:bg-white has-[:checked]:text-[#FF34C6] border-2 border-2 border-transparent select-none rounded-[8px] py-1 px-5 bg-[#FF34C6] text-white"
                            style="box-shadow: 2px 4px 5px rgb(177, 177, 177)">
                            Wanita
                    ...
                    <div>
                        ...
                        <select id="ukuran" name="ukuran" class="outline-none border rounded border-black px-2 bg-transparent">
                    ...
                    <div>
                        ...
                        <input id="stok" name="stok" class="outline-none border rounded border-black px-2 mb-3 w-24"
                            type="number" />
                    </div>
            ...
            <div>
                <button type="submit"
                    class="w-[181px] h-[59px] bg-[#2BCB4E] text-xl font-bold text-white rounded-xl border-white border" style="box-shadow: 2px 4px 6px 0 gray">Simpan</button>
            </div>
            </form>
        </div>
                        ...
                        <tr data-seragam="{{ $seragam['id'] }}" class="seragam-row divide-x-2 divide-black">
                            ...
                            <td class="px-4 ">
                              <div class="flex gap-2">  
                                <div>
                                  <button class=" text-blue-500 hover:underline" id="edit-button-{{ $seragam['id'] }}">Edit</button>
                                </div>
                                <div>
                                  <h1>/</h1>
                                </div>
                        ...
                        @if (count($seragam['semua_ukuran']) > 0)
                            @foreach ($seragam['semua_ukuran'] as $ukuran)
                                <tr data-seragam="{{ $ukuran['id'] }}" class="seragam-row divide-x-2 divide-black">
                                    <td id="nama-barang-{{ $ukuran['id'] }}" class="hidden">{{ $ukuran['nama_barang'] }}</td>
                                    <td id="ukuran-{{ $ukuran['id'] }}" class="px-4 border-2 border-black">{{ $ukuran['ukuran'] }}</td>
                                    <td id="stok-{{ $ukuran['id'] }}" class="px-4">{{ $ukuran['stok'] }}</td>
                                    <td id="harga-{{ $ukuran['id'] }}" class="px-4">{{ $ukuran['harga'] }}</td>
                                    <td class="px-4">
                                      <div class="flex gap-2">
                                        <div>
                                          <button class="edit-button text-blue-500 hover:underline" id="edit-button-{{ $ukuran['id'] }}">Edit</button>
                                        </div>
                                        <div>
                                          <h1>/</h1>
                                        </div>
    ...
    <script>
      document.querySelectorAll(".seragam-row").forEach(function(row){
        ...
        button.addEventListener("click", function(){
            ...
            document.querySelector("input[name='nama_barang']").value = getVal(`#nama-barang-${seragamId}`).trim()
            document.querySelector("select[name='ukuran']").value = getVal(`#ukuran-${seragamId}`).trim()
